...
- 작업자3,4도 git add. 까지 모두 마친상황. (각자의 로컬 저장소에서 변경된 파일의 상태를 **스테이징 상태**로 넘긴 상황)

[해결 과정]
- 마주한 에러1 : failed to push some refs to ‘원격 repository주소’
...
<br/><br/>
- 마주한 에러2 : Your local changes to the follwoing files would be overwritten by merge: ‘파일명’
- **please commit your changes or stash them before you merge** 안내 문구에 따라 스테이싱 되어있던 상태를 잠시 임시 저장소로 옮겨두고, 로컬 저장소에 원격 저장소의 가장최근상태의 branch를 로컬저장소로 merge 될 수 있도록 해주면 된다.
<br/><br/>
 ## 2) 방현영👧
  [문제 상황]
